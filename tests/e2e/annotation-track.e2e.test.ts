...
test.describe('annotation track with loaded trace', () => {
  ...
  test('renders annotation chips and keeps viewport-windowed visibility metadata', async ({ page }) => {
    ...
    const orfChip = track.locator('.annotation-chip', { hasText: 'ORF +2 88-103' })
    await expect(orfChip).toBeVisible()
    await expect(orfChip).toHaveAttribute('data-feature-type', 'orf')
    await expect(orfChip).toHaveAttribute('data-frame', '2')
    await expect(orfChip).toHaveAttribute('data-start', '88')
    await expect(orfChip).toHaveAttribute('data-end', '103')

    const restrictionChip = track.locator('.annotation-chip', { hasText: 'HindIII 251-257' })
    await expect(restrictionChip).toBeVisible()
    await expect(restrictionChip).toHaveAttribute('data-feature-type', 'restriction')
    await expect(restrictionChip).toHaveAttribute('data-enzyme', 'HindIII')
    await expect(restrictionChip).toHaveAttribute('data-start', '251')
    await expect(restrictionChip).toHaveAttribute('data-end', '257')
...
test('does not render annotation track before a trace is loaded', async ({ page }) => {
  await page.goto('')
